// src/methods/posts/getUsersOriginalPosts.js
...
/**
 * Obtiene todas las publicaciones originales de un usuario específico,
 * manejando la paginación, eliminando duplicados, y agregando la reputación real del autor.
 * Excluye la reputación cruda de la API.
 *
 * @param {string} accountName - El nombre de la cuenta para la que se desean obtener las publicaciones originales.
 * @returns {Promise<Array<object>>} Una lista de objetos de publicaciones originales, sin duplicados, con la reputación del autor calculada.
 */
export async function getUsersOriginalPosts(accountName) {
    let originalPosts = [];
    ...
    while (true) {
        ...
        for (const entry of result) {
            ...
            if (postPermlink && !processedPermlinks.has(postPermlink)) {
                ...
                const postAuthor = entry.comment ? entry.comment.author : 'N/A';
                const rebloggedOn = entry.reblogged_on === '1970-01-01T00:00:00' ? null : entry.reblogged_on;

                if (postAuthor === accountName && rebloggedOn === null) {
                    ...
                    // Obtener y añadir la reputación real del autor del post
                    try {
                        ...
                        console.warn(`Advertencia: No se pudo obtener la reputación para el autor '${postAuthor}' del post '${postPermlink}': ${repError.message}`);
                        postObject.author_reputation_calculated = null;
                    }
                    originalPosts.push(postObject);
                }
            }
        }
...
async function testMethod() {
    ...
    console.log(`Buscando publicaciones ORIGINALES de '${accountToTest}' y calculando su reputación...`);
    const posts = await getUsersOriginalPosts(accountToTest);

    if (posts && posts.length > 0) {
        console.log(`\nSe encontraron ${posts.length} publicaciones originales para '${accountToTest}':`);
        posts.forEach((post, index) => {
            ...
            console.log(`  Reputación del Autor (Calculada): ${post.author_reputation_calculated}`);
            console.log('---');
        ...
        console.log(`\nNo se encontraron publicaciones originales para '${accountToTest}'.`);
    }
